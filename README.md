...
Аннотация: Данный проект позволит тебе познакомиться со структурами и объединениями в С, а также с колбэк-функцией и дополнительно с вызовами, переменными, списками аргументов и динамическими структурами.
...
Ты замечаешь, что рядом с кучей листов на столе лежит оставленная кем-то газетная вырезка. Оставил ли ее ИИ каким-то образом, или это был кто-то до тебя? Хотя странно было бы предположить, что ИИ читает газеты и тем более делает газетные вырезки.
...
#### Получен Quest 2. Разработать программу `src/list.c`, в которой содержатся реализации функций по работе с односвязным списком дверей (`struct nodeinit(struct door door)` — инициализация структуры односвязного списка на базе переданной `struct door`, `struct nodeadd_door(struct node elem, struct doordoor)` — вставка нового элемента `struct door` в односвязный список после переданного узла `elem` , `struct node find_door(int door_id, struct noderoot)` — поиск двери в списке по ее id, `struct node remove_door(struct nodeelem, struct node root)` — удаление элемента списка, `void destroy(struct node*root)` — освобождение памяти, занимаемой структурой списка). Объявления этих функций и структуру односвязного списка вынести в файл `src/list.h`. Написать модульные тесты для проверки работы функций `add_door` и `remove_door` односвязного списка в `src/list_test.c`. Функции методов должны возвращать SUCCESS в случае успешного прохождения теста и FAIL в случае ошибки. Тесты должны собираться при помощи Makefile. Имя стадии — `list_test`. Исполняемый файл должен располагаться в корне репозитория в папке build и иметь имя «Quest_2». Обрати внимание: этот квест будет тестироваться утилитой `cppcheck`.
...
Вроде бы все сделано по структурам, можно перейти к изучению колбэк-функции.
...
Домыслить тебе не удается, так как дверь из комнаты с грохотом, похожим на взрыв, следает с петель и падает в сторону дерева.

«Что ж, выход открыт, компьютер сломан. Отчет, к сожалению, дочитать не удалось. А ИИ все так же куда-то запропастился. Надеюсь, он и не вернется. С дверьми получается прекрасно разбираться и без него», — думаешь ты и делаешь шаг через порог. 
