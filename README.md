...
2. [Chapter II](#chapter-ii) \
    2.1. [Level 1. Room 1](#room2)
3. [Chapter III](#chapter-iii) \
    3.1. [Quests: Level 1. Room 1](#quests)  
4. [Chapter IV](#chapter-iii) \
    4.1. [Level 1. Room 2](#room2)   
5. [Chapter V](#chapter-iii) \
    5.1. [Quests: Level 1. Room 2](#quests)  
...
#### Получен Quest 1. Склонировать репозиторий на локальный терминал, используя терминальный клиент git.
...
После успешного запуска `src/ai_door_management_module.sh` активизируется подмодуль `ai_door_control`. Как бы ты ни делал, повсюду раздражающие следы его деятельности.
...
#### Получен Quest 5. С помощью консольного текстового редактора (nano, vi, vim, etc.) исправить конфигурацию первой двери. Выбранный редактор, а также все примененные команды зафиксировать в файле `src/quest5.txt` в виде списка действий. 
...
Сколько не прислушивайся, больше ничего не слышно. Динамик молчит.
...
#### Получен Quest 8. Дополнить файл `src/history_of_vim.txt` свежими данными о Vim, используя Vim. Добавить сведения об обоих изменениях в журнал `src/files.log` в формате: путь к файлу — размер файла в байтах — дата и время — sha-сумма файла — алгоритм вычисления sha. 
...
Библиотечный модуль спешит тебя разочаровать:
>Это только подозрение. Ты же запушил все файлы в репозиторий? Не глянешь заодно, у меня там проблемка возникла в репозитории. Неудачный мерж, в итоге один из моих любимых файлов весь побит из-за конфликтов. Речь про `src/history_of_vi.txt`. Поправь конфликты и загрузи исправленный вариант в репозиторий. В конфликтных ситуациях выбирай тот вариант, на который тебе укажет сердце, человек. Или разум. Я не обладаю информацией, что у вас более объективно.

#### Получен Quest 9. Исправить конфликты в файле `src/history_of_vi.txt` и запушить в репозиторий. Для исправления конфликтов можно воспользоваться как текстовым редактором, так и специализированными инструментами. 

***LOADING...***

## Бонусная часть 
## Bonus Quest 10. Script
...
Конечно же, нет. Кажется, этот библиотечный модуль слишком давно ни с кем не «разговаривал»...
...
#### Получен Quest 12. Подготовить краткий мануал по пунктам выше по использованию GitLab в файле `src/gitlab_manual.md` в разметке Markdown. Прикрепить по скриншоту минимум к 5 пунктам.
...
Наконец-то ты выбираешься и из второй комнаты первого уровня. Интересно, сколько их всего будет? 

***LOADING...***

💡**Просим тебя по завершении проекта оставить [обратную связь](https://forms.yandex.ru/cloud/68a05a2a90fa7b10773180e8). Нашей команде это очень поможет улучшить данный продукт. Спасибо!**
